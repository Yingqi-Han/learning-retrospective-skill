...
- Do NOT modify persistent memory, agent rules, shell profiles, `settings.json`, `hooks.json`, or any global configuration. The only thing you may write is the target skill directory (and, only on explicit request, a copied hook script file).
...
- If and only if the user explicitly asks for hooks: read `SECURITY_NOTES.md` and `learning-retrospective/references/hook-activation.md` first, show the user the exact config change before applying it, and remind them that Codex additionally requires trusting the hook via `/hooks` in the app.
